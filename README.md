...
Line search is available as a small step-control tool, not as a broad
...
Trace history is optional and disabled by default. Use
`debug={"history": True}` in Python or `collect_trace = true` in Rust when
inspecting solver behavior.
